...
4. **Run verification tests** (if available)
   - `pnpm test -- --grep translation` (future test suite from STORY-011)
...
**Note:** Translation verification tests are planned for STORY-011. Once implemented, this command will verify:
- All tokens referenced in MDX exist in `panda.config.ts`
- All components referenced exist in `src/components/index.ts`
- All variant values match recipe definitions
