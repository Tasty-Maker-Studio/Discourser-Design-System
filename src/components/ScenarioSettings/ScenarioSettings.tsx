...
const DURATION_OPTIONS = [
  ...
  { value: 'standard', label: 'Standard (15 - 25 min)', badgeLabel: '~ (15-25 min)' },
  { value: 'extended', label: 'Extended (25 - 35 min)', badgeLabel: '~ (25-35 min)' },
] as const
...
export function ScenarioSettings({
  ...
  const [questionCount, setQuestionCount] = useState<QuestionCountValue>(defaultQuestionCount)

  const durationBadge =
    DURATION_OPTIONS.find((o) => o.value === duration)?.badgeLabel ?? '~ (10-15 min)'
  const questionBadge =
    QUESTION_OPTIONS.find((o) => o.value === questionCount)?.badgeLabel ?? '2-4 questions'
  ...
  function resolveDialogContent(sectionId: AdjustableSectionId): ReactNode {
    ...
    if (adjustmentDialogContent?.[sectionId]) return adjustmentDialogContent[sectionId]
    return null
  }
                    ...
                    <Dialog.Root>
                      ...
                      <Dialog.Positioner>
                        <Dialog.Content css={{ maxW: 'lg', w: 'full' }}>
                          ...
                          >
                            <Dialog.Title css={{ fontSize: 'xl', fontWeight: 'semibold' }}>
        ...
        <Accordion.Item value="duration" className={styles.section}>
          ...
          <Accordion.ItemContent className={styles.sectionContent}>
            <div className={styles.radioPanel}>
              <div>
                <p className={styles.radioPanelTitle}>Expected time to complete</p>
              ...
              >
                {DURATION_OPTIONS.map((option) => (
                  <RadioGroup.Item key={option.value} value={option.value}>
                    ...
                    />
                    <RadioGroup.ItemText css={{ fontSize: 'md', fontWeight: 'medium' }}>
                      {option.label}
                    </RadioGroup.ItemText>
