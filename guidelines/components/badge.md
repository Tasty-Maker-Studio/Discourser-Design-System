...
<Card>
  <div className={css({ display: 'flex', justifyContent: 'space-between', alignItems: 'start' })}>
    <Header as="h3" size="md">Premium Plan</Header>
...
<div className={css({ display: 'grid', gridTemplateColumns: '2', gap: '4' })}>
  <div>
    ...
    <Header as="h3" size="md" className={css({ mt: '2' })}>
      Dark Mode
    </Header>
...
<article aria-label="New feature: Dark mode">
  ...
  <Header as="h3" size="md">Dark Mode</Header>
</article>
...
- **Header**: Often paired with badges in headers
